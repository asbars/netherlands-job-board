...
export default function JobCard({ job, isFavorited, onToggleFavorite, isSignedIn, savedFilterLastChecked }: JobCardProps) {
  ...
  const hasSalary = job.ai_salary_minvalue || job.ai_salary_value;
      ...
      <CardContent className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-1.5">
          <Link href={`/jobs/${job.id}`} target="_blank" rel="noopener noreferrer" className="flex-1 min-w-0">
            <h3 className="text-lg font-heading font-semibold text-foreground group-hover:text-primary transition-colors leading-snug cursor-pointer">
              {job.title}
            </h3>
          </Link>
        ...
        <div className="flex flex-wrap items-center gap-1.5 mb-3">
          {job.cities_derived && job.cities_derived.length > 0 && (
            <span className="flex items-center text-xs text-muted-foreground">
              <svg className="w-3.5 h-3.5 mr-0.5 text-primary/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {Array.isArray(job.cities_derived)
                ? (() => {
                    ...
                    return sortedCities.slice(0, 2).join(', ') +
                      (sortedCities.length > 2 ? ` +${sortedCities.length - 2}` : '');
                  })()
          ...
          {job.remote_derived && (
            <Badge className="bg-primary/10 text-primary border-primary/20 text-[10px] px-2 py-0.5 hover:bg-primary/10">
              Remote
            </Badge>
          )}
        </div>
        ...
        {hasDetails && (
          <>
            {expanded && (
              <div className="text-sm text-muted-foreground space-y-2 mb-3 pt-3 border-t border-border/50">
