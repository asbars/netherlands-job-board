-- ========================================
-- V10: Add salary conversion support to search function
-- ========================================

-- Drop existing function
DROP FUNCTION IF EXISTS public.search_jobs_with_filters(jsonb, integer, integer);

-- RPC function to search jobs with array text filter support and salary conversion
...
BEGIN
  ...
  FOR v_filter IN SELECT * FROM jsonb_array_elements(p_filters) LOOP
    ...
    v_salary_unit := v_filter->>'salary_unit';
    ...
    -- Special handling for salary fields with unit conversion
    IF v_is_salary_field AND v_salary_unit IS NOT NULL THEN
      -- Build expression that converts the database salary to the target unit
      -- Use ai_working_hours if available, default to 40
      ...
      CASE v_operator
        ...
        ELSE
          -- Unknown operator for salary, skip
          NULL;
      END CASE;
