-- ========================================
-- FIXED VERSION: Handle JSONB array columns
-- ========================================
    ...
    CASE v_operator
      ...
      WHEN 'is_any_of' THEN
        IF v_value_array IS NOT NULL AND array_length(v_value_array, 1) > 0 THEN
          IF v_is_array_field THEN
            -- For JSONB array fields, use ?| operator (any match)
            -- Build proper JSONB array literal
            v_jsonb_literal := (
              SELECT jsonb_agg(elem)::text
              FROM unnest(v_value_array) AS elem
            );
            v_where_clauses := array_append(v_where_clauses,
              format('%I ?| %s', v_field, v_jsonb_literal));
          ELSE
            ...
            v_where_clauses := array_append(v_where_clauses,
              format('%I IN (%s)', v_field, v_jsonb_literal));
          END IF;
        END IF;
      ...
      WHEN 'is_not_any_of' THEN
        IF v_value_array IS NOT NULL AND array_length(v_value_array, 1) > 0 THEN
          IF v_is_array_field THEN
            ...
            v_where_clauses := array_append(v_where_clauses,
              format('NOT (%I ?| %s)', v_field, v_jsonb_literal));
          ELSE
            ...
            v_where_clauses := array_append(v_where_clauses,
              format('%I NOT IN (%s)', v_field, v_jsonb_literal));
          END IF;
        END IF;
...
-- ========================================
-- Test with is_any_of filter on City (Dubai)
SELECT * FROM public.search_jobs_with_filters(
  '[{"field":"cities_derived","operator":"is_any_of","value":["Dubai"],"is_array_value":true}]'::jsonb,
  1,
  5
);
