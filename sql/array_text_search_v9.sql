-- ========================================
-- V9: Replace N/A with Other (includes empty AND "OTHER" values)
-- ========================================
    ...
    CASE v_operator
      ...
      WHEN 'is_any_of' THEN
        IF v_value_array IS NOT NULL AND array_length(v_value_array, 1) > 0 THEN
          ...
          -- Special handling for ai_employment_type field (text[] array)
          ELSIF v_field = 'ai_employment_type' THEN
            -- Check if "Other" is in the filter values
            v_includes_other := 'Other' = ANY(v_value_array);

            -- Get non-Other values
            SELECT array_agg(elem) INTO v_non_other_values
            FROM unnest(v_value_array) AS elem
            WHERE elem != 'Other';

            -- Build condition based on whether "Other" is included
            IF v_includes_other AND v_non_other_values IS NOT NULL AND array_length(v_non_other_values, 1) > 0 THEN
              -- Both "Other" and real values: match NULL/empty OR "OTHER" OR any of the real values using && operator for text[]
            ...
            ELSIF v_includes_other THEN
              -- Only "Other" selected: match NULL, empty array, or "OTHER"
              v_where_clauses := array_append(v_where_clauses,
                format('(%I IS NULL OR array_length(%I, 1) IS NULL OR %I && ARRAY[''OTHER'']::text[])', v_field, v_field, v_field));
            ELSE
              -- No "Other": standard text[] array matching using && operator
              v_array_literal := 'ARRAY[' || (
                SELECT string_agg(quote_literal(elem), ',')
                FROM unnest(v_value_array) AS elem
              ) || ']::text[]';
...
-- ========================================
-- TEST: Test with "Other" filter
-- ========================================
-- This should return jobs with NULL, empty ai_employment_type, OR "OTHER"
SELECT * FROM public.search_jobs_with_filters(
  '[{"field":"ai_employment_type","operator":"is_any_of","value":["Other"],"is_array_value":true}]'::jsonb,
  1,
  10
);

-- Test with "Other" and Full-time
SELECT * FROM public.search_jobs_with_filters(
  '[{"field":"ai_employment_type","operator":"is_any_of","value":["Other","FULL_TIME"],"is_array_value":true}]'::jsonb,
  1,
  10
);
