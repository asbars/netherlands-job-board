-- ========================================
-- FIXED VERSION: Proper array handling for PostgreSQL operators
-- ========================================
    ...
    CASE v_operator
      ...
      WHEN 'is_any_of' THEN
        IF v_value_array IS NOT NULL AND array_length(v_value_array, 1) > 0 THEN
          IF v_is_array_field THEN
            -- For array fields, build proper array literal string
            -- Convert array to PostgreSQL array literal: ARRAY['val1','val2']
      ...
      WHEN 'is_not_any_of' THEN
        IF v_value_array IS NOT NULL AND array_length(v_value_array, 1) > 0 THEN
          IF v_is_array_field THEN
            -- For array fields, check arrays don't overlap
            v_array_literal := 'ARRAY[' || (
              SELECT string_agg(quote_literal(elem), ',')
              FROM unnest(v_value_array) AS elem
            ) || ']::text[]';
